...
{
    switch (type) {
        case ShaderType::COLOR: case ShaderType::LIGHTING_COLOR:
            return "";
        case ShaderType::MESH: case ShaderType::LIGHTING_MESH:
            return R"(
layout (location = 1) in vec4 color;
out vec4 color_;
)";
        case ShaderType::TEXTURE: case ShaderType::LIGHTING_TEXTURE:
            return R"(
...
std::string multilight_vert_body_perlight(int num_lights)
{
    ...
    for (int i = 0; i < num_lights; i += 1) {
        ...
        s += "    "s + li + "_dir = "s + li + "_eye.xyz - vert_eye.xyz * "s + li + "_eye.a;\n"s;
...
std::string multilight_vert_shader(ShaderType type, int num_lights, bool is_omni)
{
    return al_default_shader_version_string()
         ...
         + multilight_vert_body_pertype(type)
         + multilight_vert_body_perlight(num_lights)
         + multilight_vert_body_end();
}
...
std::string multilight_frag_shader(ShaderType type, int num_lights)
{
    return multilight_frag_header_common()
         ...
         + multilight_frag_body_begin()
         + multilight_frag_body_pertype(type)
         + multilight_frag_body_perlight(num_lights)
