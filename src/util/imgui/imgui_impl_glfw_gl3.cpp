...
void ImGui_ImplGlfwGL3_NewFrame()
{
    ...
    if (glfwGetWindowAttrib(g_Window, GLFW_FOCUSED))
    {
        ...
        {
            glfwSetCursorPos(g_Window, (double)io.MousePos.x, (double)io.MousePos.y);   // Set mouse position if requested by io.WantSetMousePos flag (used when io.NavMovesTrue is enabled by user and using directional navigation)
        }
        else
        {
